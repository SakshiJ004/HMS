...
const Appointments = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      console.log('✅ Appointments loaded:', transformedData.length);
    } catch (error) {
          ...
          <div className="table-responsive">
            {appointments.length === 0 ? (
              <div className="text-center py-5">
                <i className="ti ti-calendar-off" style={{ fontSize: '48px', color: '#ccc' }} />
                <p className="text-muted mt-3">No appointments found</p>
                <Link to={all_routes.newAppointment} className="btn btn-primary mt-2">
                  <i className="ti ti-plus me-1" /> Create First Appointment
                </Link>
              </div>
            ) : (
              <Datatable
