...
interface ServicesModalProps {
  ...
  onCloseEdit: () => void;
  onCloseDelete: () => void;
  onAdd: (name: string, department: string, price: string, status: string) => void;
  onEdit: (name: string, department: string, price: string, status: string) => void;
  onDelete: () => void;
}
...
const ServicesModal = ({
  ...
  // Update edit form when currentService changes
  useEffect(() => {
    if (currentService && showEditModal) {
      setEditFormData({
  ...
  }, [currentService, showEditModal]);

  // Handle Add Submit
  const handleAddSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (addFormData.name.trim() && addFormData.department && addFormData.price) {
      onAdd(addFormData.name, addFormData.department, addFormData.price, addFormData.status);
      setAddFormData({ name: "", department: Department[0]?.value || "", price: "", status: "Active" });
    }
  };
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editFormData.name.trim() && editFormData.department && editFormData.price) {
      onEdit(editFormData.name, editFormData.department, editFormData.price, editFormData.status);
    }
  };
              ...
              <div className="modal-body">
                ...
                <div className="mb-3">
                  ...
                  <CommonSelect
                    ...
                    defaultValue={Department[0]}
                    onChange={(option: any) =>
                      setAddFormData({ ...addFormData, department: option?.value || "" })
                    }
                  />
                </div>
                <div className="mb-3">
              ...
              <div className="modal-body">
                ...
                <div className="mb-3">
                  ...
                  <CommonSelect
                    options={Department}
                    className="select"
                    value={Department.find((d: any) => d.value === editFormData.department)}
                    onChange={(option: any) =>
                      setEditFormData({ ...editFormData, department: option?.value || "" })
                    }
                  />
                </div>
                <div className="mb-3">
                ...
                <div className="mb-0">
                  ...
                  <CommonSelect
                    ...
                    value={
                      editFormData.status
                        ? { label: editFormData.status, value: editFormData.status }
                        : { label: "Active", value: "Active" }
                    }
                    onChange={(option: any) =>
