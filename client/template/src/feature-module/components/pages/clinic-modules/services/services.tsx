...
const Services = () => {
  ...
  const fetchServices = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${BACKEND_URL}/api/services?sortBy=${sortBy}`, {
  ...
  const handleEditOpen = (item: Service) => {
    setSelectedItem(item);
    setShowEditModal(true);
  };

  const handleDeleteOpen = (item: Service) => {
    setSelectedItem(item);
  ...
  const filteredData = services.filter((s) => {
    ...
    const matchDept =
      ...
      filterDepartment.some((d) =>
        s.department.toLowerCase().includes(d.toLowerCase())
      );
                ...
                {loading ? (
                  ...
                  filteredData.map((s) => (
                    <tr key={s._id}>
                      ...
                      <td className="text-muted fs-13">{s.department}</td>
                      <td className="fw-semibold">${s.price.toFixed(2)}</td>
                      <td>
                        <span
