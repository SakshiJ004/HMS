...
const AddDoctor = () => {
    ...
    const fetchSpecializations = async () => {
      try {
        ...
        if (res.data.success && res.data.data.length > 0) {
          ...
          setDepartment(options[0]); // Set first option as default
        }
      } catch (err) {
  ...
  const [dob, setDob] = useState<Dayjs | null>(null);
  const [yearOfExperience, setYearOfExperience] = useState("");
  const [department, setDepartment] = useState<DepartmentOption>({ value: "", label: "" });
  const [designation, setDesignation] = useState(Designation[0]);
  const [medicalLicenseNumber, setMedicalLicenseNumber] = useState("");
  const [tags, setTags] = useState<string[]>(["English", "French"]);
  const [bloodGroup, setBloodGroup] = useState(Blood_Group[0]);
  ...
  const handleApplyAllSchedules = () => {
    const activeTab = document.querySelector('.schedule-tab .nav-link.active');
    if (!activeTab) { alert('Please select a day first'); return; }
    let sourceSchedule: Array<{ startTime: string; endTime: string }> = [];
    let dayName = '';
    if (activeTab.textContent?.includes('Monday')) { sourceSchedule = mondaySchedule; dayName = 'Monday'; }
    else if (activeTab.textContent?.includes('Tuesday')) { sourceSchedule = tuesdaySchedule; dayName = 'Tuesday'; }
    else if (activeTab.textContent?.includes('Wednesday')) { sourceSchedule = wednesdaySchedule; dayName = 'Wednesday'; }
    else if (activeTab.textContent?.includes('Thursday')) { sourceSchedule = thursdaySchedule; dayName = 'Thursday'; }
    else if (activeTab.textContent?.includes('Friday')) { sourceSchedule = fridaySchedule; dayName = 'Friday'; }
    else if (activeTab.textContent?.includes('Saturday')) { sourceSchedule = saturdaySchedule; dayName = 'Saturday'; }
    else if (activeTab.textContent?.includes('Sunday')) { sourceSchedule = sundaySchedule; dayName = 'Sunday'; }
    if (sourceSchedule.length === 0) { alert(`Please add at least one time slot for ${dayName} before applying to all days`); return; }
    if (!window.confirm(`This will copy ${dayName}'s schedule to all other days. Continue?`)) return;
    setMondaySchedule([...sourceSchedule]);
    setTuesdaySchedule([...sourceSchedule]);
    setWednesdaySchedule([...sourceSchedule]);
    setThursdaySchedule([...sourceSchedule]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sanitizeSlots = (slots: Array<{ startTime: string; endTime: string }>) =>
        slots.filter(ts => ts.startTime !== "00:00:00" && ts.endTime !== "00:00:00");
      const schedules = [];
      if (sanitizeSlots(mondaySchedule).length > 0) schedules.push({ day: "Monday", timeSlots: sanitizeSlots(mondaySchedule) });
      if (sanitizeSlots(tuesdaySchedule).length > 0) schedules.push({ day: "Tuesday", timeSlots: sanitizeSlots(tuesdaySchedule) });
      if (sanitizeSlots(wednesdaySchedule).length > 0) schedules.push({ day: "Wednesday", timeSlots: sanitizeSlots(wednesdaySchedule) });
      if (sanitizeSlots(thursdaySchedule).length > 0) schedules.push({ day: "Thursday", timeSlots: sanitizeSlots(thursdaySchedule) });
      if (sanitizeSlots(fridaySchedule).length > 0) schedules.push({ day: "Friday", timeSlots: sanitizeSlots(fridaySchedule) });
      if (sanitizeSlots(saturdaySchedule).length > 0) schedules.push({ day: "Saturday", timeSlots: sanitizeSlots(saturdaySchedule) });
      if (sanitizeSlots(sundaySchedule).length > 0) schedules.push({ day: "Sunday", timeSlots: sanitizeSlots(sundaySchedule) });

      const doctorData: DoctorFormData = {
        firstName, lastName, username, email, phone,
        dob: dob.format("YYYY-MM-DD"),
        yearOfExperience: parseInt(yearOfExperience),
        department: department.value,  // ✅ Dynamic value from API
        designation: designation.value,
        medicalLicenseNumber,
        languageSpoken: tags,
        bloodGroup: bloodGroup.value,
        gender: gender.value,
        bio, featureOnWebsite,
        address: { address1, address2, city: city.value, state: state.value, country: country.value, pincode },
        appointmentType: appointmentType.value,
        acceptBookingsDays: acceptBookingsDays ? parseInt(acceptBookingsDays) : 0,
        appointmentDuration: appointmentDuration ? parseInt(appointmentDuration) : 30,
        consultationCharge: consultationCharge ? parseFloat(consultationCharge) : 0,
                  ...
                  <form onSubmit={handleSubmit}>
                        ...
                        <div className="col-lg-12">
                          <div className="mb-3 d-flex align-items-center">
                            ...
                            <div className="drag-upload-btn avatar avatar-xxl rounded-circle bg-light text-muted position-relative overflow-hidden z-1 mb-2 ms-4 p-0">
                              ...
                              <input type="file" className="form-control image-sign" accept="image/*" onChange={handleImageUpload} />
                              <div className="position-absolute bottom-0 end-0 star-0 w-100 h-25 bg-dark d-flex align-items-center justify-content-center z-n1">
                          ...
                          <div className="row">
                            <div className="col-lg-6">
                              <div className="mb-3">
                                ...
                                {/* ✅ Dynamic dropdown from specializations API */}
                                {departmentsLoading ? (
                                  ...
                                  <CommonSelect
                                    ...
                                    className="select"
                                    value={department.value ? department : departmentOptions[0] || { value: "", label: "Select Department" }}
                                    onChange={(option: any) => option && setDepartment({ value: option.value, label: option.label })}
                                  />
                                )}
                              </div>
                            </div>
                    ...
                    <div className="p-3">
                      <ul className="nav nav-pills schedule-tab mb-3" id="pills-tab" role="tablist">
                        {[
                          { label: "Monday", target: "#schedule-1" },
                          { label: "Tuesday", target: "#schedule-2" },
                          { label: "Wednesday", target: "#schedule-3" },
                          { label: "Thursday", target: "#schedule-4" },
                          { label: "Friday", target: "#schedule-5" },
                          { label: "Saturday", target: "#schedule-6" },
                          { label: "Sunday", target: "#schedule-7" },
                        ].map((day, idx) => (
                          <li className="nav-item me-1" role="presentation" key={day.label}>
                            <button
                              className={`nav-link btn btn-sm btn-icon p-2 d-flex align-items-center justify-content-center w-auto ${idx === 0 ? "active" : ""}`}
                              data-bs-toggle="pill"
                              data-bs-target={day.target}
                              type="button"
                              role="tab"
                              aria-selected={idx === 0}
                      ...
                      <div className="tab-content" id="pills-tabContent">
                        {[
                          { id: "schedule-1", schedule: mondaySchedule, setter: setMondaySchedule },
                          { id: "schedule-2", schedule: tuesdaySchedule, setter: setTuesdaySchedule },
                          { id: "schedule-3", schedule: wednesdaySchedule, setter: setWednesdaySchedule },
                          { id: "schedule-4", schedule: thursdaySchedule, setter: setThursdaySchedule },
                          { id: "schedule-5", schedule: fridaySchedule, setter: setFridaySchedule },
                          { id: "schedule-6", schedule: saturdaySchedule, setter: setSaturdaySchedule },
                          { id: "schedule-7", schedule: sundaySchedule, setter: setSundaySchedule },
                        ].map((item, idx) => (
                          <div className={`tab-pane fade ${idx === 0 ? "active show" : ""}`} id={item.id} role="tabpanel" key={item.id}>
                            <div className="add-schedule-list">
                              <DuplicateForms onScheduleChange={item.setter} scheduleData={item.schedule} />
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="d-flex justify-content-end gap-2">
                      <button type="button" className="btn btn-light btm-md" onClick={handleCancel} disabled={loading}>Cancel</button>
      ...
      {showSuccessModal && successModalData && (
        <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header bg-success text-white">
                <h5 className="modal-title"><i className="ti ti-check-circle me-2"></i>Doctor Created Successfully!</h5>
              </div>
              <div className="modal-body">
                <div className="alert alert-warning mb-3">
                  <i className="ti ti-alert-triangle me-2"></i>
                ...
                <div className="mb-3">
                  <label className="form-label fw-bold">Email</label>
                  <input type="text" className="form-control" value={successModalData.email} readOnly />
                </div>
                <div className="mb-3">
                  <label className="form-label fw-bold">Username</label>
                  <input type="text" className="form-control" value={successModalData.username} readOnly />
                </div>
                <div className="mb-3">
                  <label className="form-label fw-bold">Password</label>
                  <input type="text" className="form-control" value={successModalData.password} readOnly />
                </div>
                <button className="btn btn-primary w-100 mb-2" onClick={handleCopyCredentials}>
                  <i className="ti ti-copy me-2"></i>Copy All Credentials
                </button>
              </div>
              <div className="modal-footer">
                <button className="btn btn-success w-100" onClick={() => { setShowSuccessModal(false); navigate(all_routes.doctors); }}>
                  Go to Doctors List
                </button>
              </div>
            </div>
