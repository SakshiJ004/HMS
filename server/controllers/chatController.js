...
exports.getConversations = async (req, res) => {
    try {
        const { myName } = req.query; // frontend sends ?myName=sakshi

        let filter = {};
        if (myName) {
            filter = { 'participants.name': myName };
        }
...
// ✅ Get messages — mark as read instantly
exports.getMessages = async (req, res) => {
    try {
        const { conversationId } = req.params;
        ...
        Message.updateMany({ conversationId, isRead: false }, { isRead: true }).exec();
        Conversation.findByIdAndUpdate(conversationId, { unreadCount: 0 }).exec();
...
// ✅ Get all users for New Chat — Staff + Doctors combined
exports.getChatUsers = async (req, res) => {
    try {
        const Staff = require('../models/Staff');
        const Doctor = require('../models/Doctor');
